...
from mintnet.experiments.stage8a_calibration import (
    CONDITIONS,
...
_SMOKE_CONFIG = Path("configs/stage8a_calibration_smoke.yaml")


def test_stage8a_calibration_covers_every_expected_combination(tmp_path: Path) -> None:
    config = load_config(_SMOKE_CONFIG)
    raw = run_stage8a(config, tmp_path / "evidence", write_report=False)

    combos = set(zip(raw["condition"], raw["n"], raw["replicate"], raw["edge"]))
    assert combos == expected_combinations(config)
    assert len(raw) == expected_row_count(config)
    assert (raw["status"] == "ok").all()


def test_stage8a_calibration_sharded_run_matches_unsharded_run(tmp_path: Path) -> None:
    config = load_config(_SMOKE_CONFIG)

    unsharded = run_stage8a(config, tmp_path / "unsharded", write_report=False)
    shard = run_stage8a(
        config, tmp_path / "shard",
        conditions=("triangle_balanced",), sample_sizes=(300,), batches=(0,), write_report=False,
    )

    expected = unsharded.loc[
        (unsharded["condition"] == "triangle_balanced") & (unsharded["n"] == 300)
...
def test_stage8a_calibration_aggregate_shards_reproduces_unsharded_report(tmp_path: Path) -> None:
    import sys
    ...
    unsharded = run_stage8a(config, unsharded_dir)
    ...
    aggregated = aggregate("mintnet.experiments.stage8a_calibration", _SMOKE_CONFIG, shards_dir, aggregated_dir)

    dedup_columns = ["condition", "n", "replicate", "edge"]
    ...
    assert (aggregated_dir / "bin_table.csv").is_file()
