...
from mintnet.experiments.stage7f_frontier import (
    _conditions,
...
_SMOKE_CONFIG = Path("configs/stage7f_frontier_smoke.yaml")


def test_stage7f_covers_every_expected_combination(tmp_path: Path) -> None:
    config = load_config(_SMOKE_CONFIG)
    raw = run_stage7f(config, tmp_path / "evidence", write_report=False)
...
def test_stage7f_sharded_run_matches_unsharded_run(tmp_path: Path) -> None:
    config = load_config(_SMOKE_CONFIG)

    unsharded = run_stage7f(config, tmp_path / "unsharded", write_report=False)
    shard = run_stage7f(
        config, tmp_path / "shard",
        conditions=("triangle_0",), sample_sizes=(750,), batches=(0,), write_report=False,
    )

    expected = unsharded.loc[
        (unsharded["condition"] == "triangle_0") & (unsharded["n"] == 750)
...
def test_stage7f_aggregate_shards_reproduces_unsharded_evidence(tmp_path: Path) -> None:
    import sys
    ...
    unsharded = run_stage7f(config, unsharded_dir)
    ...
    aggregated = aggregate("mintnet.experiments.stage7f_frontier", _SMOKE_CONFIG, shards_dir, aggregated_dir)
    ...
    assert (aggregated_dir / "frontier_table.csv").is_file()
    assert (aggregated_dir / "smallest_resolvable_target_rho.csv").is_file()
    assert (aggregated_dir / "confidence_verification.json").is_file()
