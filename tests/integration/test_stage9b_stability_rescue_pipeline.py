...
_SMOKE_CONFIG = Path("configs/stage9b_stability_rescue_validation_smoke.yaml")


def test_stage9b_covers_every_expected_combination(tmp_path: Path) -> None:
    config = load_config(_SMOKE_CONFIG)
    raw = run_stage9b(config, tmp_path / "evidence", write_report=False)
...
def test_stage9b_sharded_run_matches_unsharded_run(tmp_path: Path) -> None:
    config = load_config(_SMOKE_CONFIG)

    unsharded = run_stage9b(config, tmp_path / "unsharded", write_report=False)
    shard = run_stage9b(config, tmp_path / "shard", dgps=("overlap",), sample_sizes=(1000,), write_report=False)
...
def test_stage9b_aggregate_shards_reproduces_unsharded_report(tmp_path: Path) -> None:
    import sys
    ...
    unsharded = run_stage9b(config, unsharded_dir)
    ...
    aggregated = aggregate("mintnet.experiments.stage9b_stability_rescue_validation", _SMOKE_CONFIG, shards_dir, aggregated_dir)
    ...
    assert (aggregated_dir / "recall_removal_table.csv").is_file()
