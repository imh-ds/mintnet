"""Frozen gate evaluation and evidence rendering for the Stage 1b conditional-independence experiment."""
...
class GateDecision:
    """The immutable R2b decision derived from one run's raw evidence."""
...
def _target_sample_sizes(config: Stage1bConfig) -> tuple[int, ...]:
...
def aggregate_stage1b(raw: pd.DataFrame) -> pd.DataFrame:
...
def _development_alpha_passes(rows: pd.DataFrame, alpha: float, config: Stage1bConfig) -> bool:
...
def select_alpha_pair(raw: pd.DataFrame, config: Stage1bConfig) -> tuple[float, float] | None:
...
def evaluate_stage1b_gate(raw: pd.DataFrame, config: Stage1bConfig) -> GateDecision:
...
def compute_calibration(raw: pd.DataFrame, config: Stage1bConfig) -> pd.DataFrame:
...
def write_stage1b_report(raw: pd.DataFrame, config: Stage1bConfig, output_dir: Path) -> GateDecision:
    """Write all aggregate evidence and the frozen R2b decision for one run."""
