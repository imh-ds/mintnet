"""Deterministic raw-evidence runner for the frozen Stage 1b conditional-independence experiment."""
...
def load_stage1b_config(path: Path) -> Stage1bConfig:
    """Load a Stage 1b configuration, retaining the path used to resolve it."""
    with path.open(encoding="utf-8") as stream:
        values = yaml.safe_load(stream)
    if not isinstance(values, dict):
        raise ValueError("Stage 1b configuration must be a mapping")
...
def _condition_seed(
    ...
    # Matches Stage 1's seed derivation exactly so both charters simulate the
    # same underlying data, isolating the pruning mechanism as the only
    # difference between the R2 and R2b evidence.
...
def _write_evidence(
    config: Stage1bConfig, output_dir: Path, raw: pd.DataFrame, runtime_seconds: float
) -> None:
    ...
    charter = repository_root / "docs/stage1b_charter.md"
...
def run_stage1b(config: Stage1bConfig, output_dir: Path) -> pd.DataFrame:
    """Run configured Stage 1b conditions and persist only raw evidence."""
    ...
    # every CLI invocation leaves the aggregate R2b gate evidence alongside it.
    from mintnet.experiments.stage1b_reporting import write_stage1b_report

    write_stage1b_report(raw, config, output_dir)
    return raw
...
def main() -> None:
    ...
    run_stage1b(load_stage1b_config(arguments.config), arguments.output)
