"""Deterministic raw-evidence runner for the frozen Stage 2d overlap-wiring experiment.

DGP: chain (0-2), measured fork (3-5), the shared-node-overlap motif
(6-10, node 8 shared between two triangles), and noise (11-14) -- a
single p=15 network testing whether the shared-node-overlap shape (now
validated for isolated conditioning at D-017) works when wired into the
composed pipeline, where screening's own detection reliability for its
weak cross-branch correlation becomes a real, DGP-dependent factor. See
docs/stage2d_charter.md.
"""
...
TRUE_CANDIDATE_PAIRS: frozenset[tuple[int, int]] = frozenset(
    ...
    | set(combinations((6, 7, 8, 9, 10), 2))
)
TRUE_DIRECT_EDGES: frozenset[tuple[int, int]] = frozenset(
    {(0, 1), (1, 2), (3, 4), (4, 5), (6, 7), (6, 8), (7, 8), (8, 9), (8, 10), (9, 10)}
)
CHAIN_INDIRECT: tuple[tuple[int, int], ...] = ((0, 2),)
FORK_INDIRECT: tuple[tuple[int, int], ...] = ((3, 5),)
OVERLAP_INDIRECT: tuple[tuple[int, int], ...] = ((6, 9), (6, 10), (7, 9), (7, 10))
MOTIF_COMPONENTS: dict[str, frozenset[int]] = {
    "chain": frozenset({0, 1, 2}),
    "fork": frozenset({3, 4, 5}),
    "overlap": frozenset({6, 7, 8, 9, 10}),
}
OVERLAP_PAIRS: tuple[tuple[int, int], ...] = tuple(combinations((6, 7, 8, 9, 10), 2))
NOISE_COUNT = 4
P = 11 + NOISE_COUNT
...
def load_stage2d_config(path: Path) -> Stage2dConfig:
    """Load a Stage 2d configuration, retaining the path used to resolve it."""
    with path.open(encoding="utf-8") as stream:
        values = yaml.safe_load(stream)
    if not isinstance(values, dict):
        raise ValueError("Stage 2d configuration must be a mapping")
...
def _condition_seed(config: Stage2dConfig, sample_index: int, replicate: int) -> int:
...
def _write_evidence(config: Stage2dConfig, output_dir: Path, raw: pd.DataFrame, runtime_seconds: float) -> None:
    ...
    charter = repository_root / "docs/stage2d_charter.md"
...
def _score(screened: np.ndarray, final: np.ndarray, p: int) -> dict[str, float]:
    ...
    return {
        "chain_indirect_tpr": _tpr(final, CHAIN_INDIRECT),
        "fork_indirect_tpr": _tpr(final, FORK_INDIRECT),
...
def run_stage2d(config: Stage2dConfig, output_dir: Path) -> pd.DataFrame:
    """Run configured Stage 2d conditions and persist only raw evidence."""
    ...
    from mintnet.experiments.stage2d_reporting import write_stage2d_report

    write_stage2d_report(raw, config, output_dir)
    return raw
...
def main() -> None:
    ...
    run_stage2d(load_stage2d_config(arguments.config), arguments.output)
