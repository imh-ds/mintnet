...
from mintnet.experiments.stage7e_isolation import (
    CONDITIONS,
...
def test_stage7e_isolation_covers_every_expected_combination(tmp_path: Path) -> None:
    config = load_config(_SMOKE_CONFIG)
    raw = run_stage7e_isolation(config, tmp_path / "evidence", write_report=False)
...
def test_stage7e_isolation_recovers_chain_and_triangle_structure_at_a_lenient_alpha(tmp_path: Path) -> None:
...
def test_stage7e_isolation_sharded_run_matches_unsharded_run(tmp_path: Path) -> None:
    config = load_config(_SMOKE_CONFIG)

    unsharded = run_stage7e_isolation(config, tmp_path / "unsharded", write_report=False)
    shard = run_stage7e_isolation(
...
def test_stage7e_isolation_aggregate_shards_reproduces_unsharded_report(tmp_path: Path) -> None:
    import sys
    ...
    unsharded = run_stage7e_isolation(config, unsharded_dir)
    ...
    aggregated = aggregate("mintnet.experiments.stage7e_isolation", _SMOKE_CONFIG, shards_dir, aggregated_dir)
    ...
    assert (aggregated_dir / "stage7e_isolation_report.md").is_file()
