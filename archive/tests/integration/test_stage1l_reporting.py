...
from mintnet.experiments.stage1l_reporting import evaluate_stage1l_gate, write_stage1l_report


def _config() -> Stage1lConfig:
...
def test_gate_requires_margin_at_least_the_configured_threshold():
    decision = evaluate_stage1l_gate(_raw_rows(), _config())
...
def test_report_writes_required_evidence(tmp_path: Path) -> None:
    decision = write_stage1l_report(_raw_rows(), _config(), tmp_path)
    assert len(decision.by_n) == 2
    for filename in ("decision.json", "stage1l_report.md"):
