...
from mintnet.experiments.stage1d_reporting import select_alpha_pair, write_stage1d_report


def _config() -> Stage1dConfig:
    return Stage1dConfig(
...
def test_report_writes_passing_decision_and_required_evidence(tmp_path: Path) -> None:
    decision = write_stage1d_report(_raw_rows(), _config(), tmp_path)
