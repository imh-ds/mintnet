...
def test_stage1l_smoke_runner_is_deterministic(tmp_path: Path) -> None:
    config = load_stage1l_config(Path("configs/stage1l_overlap_smoke.yaml"))

    first = run_stage1l(config, tmp_path / "first")
    second = run_stage1l(config, tmp_path / "second")
...
def test_stage1l_metrics_are_in_range(tmp_path: Path) -> None:
    config = load_stage1l_config(Path("configs/stage1l_overlap_smoke.yaml"))
    raw = run_stage1l(config, tmp_path / "evidence")
...
def test_stage1l_provenance_uses_config_repository_when_cwd_changes(
    tmp_path: Path, monkeypatch
) -> None:
    repository_root = Path(__file__).resolve().parents[2]
    config = load_stage1l_config((repository_root / "configs/stage1l_overlap_smoke.yaml").resolve())
    output = (tmp_path / "evidence").resolve()
    expected_hash = hashlib.sha256(
        (repository_root / "docs/stage1l_charter.md").read_bytes()
    ).hexdigest()
