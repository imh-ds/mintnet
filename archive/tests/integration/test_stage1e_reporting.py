...
from mintnet.experiments.stage1e_reporting import select_alpha_pair, write_stage1e_report


def _config() -> Stage1eConfig:
    return Stage1eConfig(
...
def test_report_writes_passing_decision_and_required_evidence(tmp_path: Path) -> None:
    decision = write_stage1e_report(_raw_rows(), _config(), tmp_path)
