...
from mintnet.experiments.stage1h_reporting import evaluate_stage1h_gate, write_stage1h_report


def _config() -> Stage1hConfig:
    return Stage1hConfig(
...
def test_each_n_is_selected_and_gated_independently():
    ...
    decision = evaluate_stage1h_gate(_raw_rows(), _config())
...
def test_report_writes_a_per_n_table(tmp_path: Path) -> None:
    decision = write_stage1h_report(_raw_rows(), _config(), tmp_path)
    ...
    report_text = (tmp_path / "stage1h_report.md").read_text(encoding="utf-8")
