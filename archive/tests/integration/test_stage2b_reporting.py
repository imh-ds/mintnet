...
from mintnet.experiments.stage2b_reporting import evaluate_stage2b_gate, write_stage2b_report


def _config() -> Stage2bConfig:
...
def test_gate_passes_when_no_regression_and_thresholds_met():
    decision = evaluate_stage2b_gate(_raw_rows(), _config())
...
def test_report_writes_required_evidence(tmp_path: Path) -> None:
    decision = write_stage2b_report(_raw_rows(), _config(), tmp_path)
    assert len(decision.by_n) == 2
    for filename in ("decision.json", "stage2b_report.md", "false_edge_rate_comparison.png"):
