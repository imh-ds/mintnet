...
from mintnet.experiments.stage2c import load_stage2c_config, run_stage2c


def test_stage2c_smoke_runner_is_deterministic(tmp_path: Path) -> None:
    config = load_stage2c_config(Path("configs/stage2c_composition_smoke.yaml"))

    first = run_stage2c(config, tmp_path / "first")
    second = run_stage2c(config, tmp_path / "second")
...
def test_stage2c_metrics_reflect_ground_truth(tmp_path: Path) -> None:
    config = load_stage2c_config(Path("configs/stage2c_composition_smoke.yaml"))
    raw = run_stage2c(config, tmp_path / "evidence")
...
def test_stage2c_provenance_uses_config_repository_when_cwd_changes(
    tmp_path: Path, monkeypatch
) -> None:
    repository_root = Path(__file__).resolve().parents[2]
    config = load_stage2c_config((repository_root / "configs/stage2c_composition_smoke.yaml").resolve())
