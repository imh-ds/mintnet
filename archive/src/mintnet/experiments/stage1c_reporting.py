"""Frozen gate evaluation and evidence rendering for the Stage 1c conditional-independence experiment."""
...
class GateDecision:
    """The immutable R2c decision derived from one run's raw evidence."""
...
def _target_sample_sizes(config: Stage1cConfig) -> tuple[int, ...]:
    """Use the frozen large-N gate cells that are present in this configuration.

    Per docs/stage1c_charter.md, the gate floor is N >= 750, raised from
...
def aggregate_stage1c(raw: pd.DataFrame) -> pd.DataFrame:
...
def _pooled_metric(rows: pd.DataFrame, motif: str, alpha: float, metric: str) -> float | None:
    values = rows.loc[(rows["motif"] == motif) & (rows["alpha"] == alpha), metric]
    if values.empty or not np.isfinite(values).all():
        return None
    return float(values.mean())


def _development_alpha_passes(rows: pd.DataFrame, alpha: float, config: Stage1cConfig) -> bool:
    chain_tpr = _pooled_metric(rows, "chain", alpha, "indirect_prune_tpr")
    fork_tpr = _pooled_metric(rows, "fork", alpha, "indirect_prune_tpr")
    triangle_fpr = _pooled_metric(rows, "triangle", alpha, "true_edge_prune_fpr")
    return (
...
def select_alpha_pair(raw: pd.DataFrame, config: Stage1cConfig) -> tuple[float, float] | None:
...
def evaluate_stage1c_gate(raw: pd.DataFrame, config: Stage1cConfig) -> GateDecision:
...
def compute_calibration(raw: pd.DataFrame, config: Stage1cConfig) -> pd.DataFrame:
...
def write_stage1c_report(raw: pd.DataFrame, config: Stage1cConfig, output_dir: Path) -> GateDecision:
    """Write all aggregate evidence and the frozen R2c decision for one run."""
    output_dir.mkdir(parents=True, exist_ok=True)
    aggregate = aggregate_stage1c(raw)
    aggregate.to_csv(output_dir / "aggregate_metrics.csv", index=False)
    decision = evaluate_stage1c_gate(raw, config)
    ...
    (output_dir / "stage1c_report.md").write_text(
        "# Stage 1c Conditional-Independence Motif Validation Report\n\n"
