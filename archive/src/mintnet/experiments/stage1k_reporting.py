"""Held-out gate evaluation and evidence rendering for the Stage 1k experiment."""
...
def evaluate_n(raw: pd.DataFrame, n: int, config: Stage1kConfig) -> NDecision:
...
def evaluate_stage1k_gate(raw: pd.DataFrame, config: Stage1kConfig) -> GateDecision:
    return GateDecision(tuple(evaluate_n(raw, n, config) for n in config.sample_sizes))


def write_stage1k_report(raw: pd.DataFrame, config: Stage1kConfig, output_dir: Path) -> GateDecision:
    output_dir.mkdir(parents=True, exist_ok=True)
    decision = evaluate_stage1k_gate(raw, config)
    ...
    table = "\n".join(rows)
    (output_dir / "stage1k_report.md").write_text(
        "# Stage 1k Multi-Variable Conditioning Report\n\n"
        f"{table}\n\n"
