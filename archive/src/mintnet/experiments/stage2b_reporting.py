...
def evaluate_n(raw: pd.DataFrame, n: int, config: Stage2bConfig) -> NDecision:
    ...
    triad_rate = float(
        validation[["chain_is_triad", "fork_is_triad", "triangle_is_triad"]].mean().mean()
    )
...
def evaluate_stage2b_gate(raw: pd.DataFrame, config: Stage2bConfig) -> GateDecision:
...
def write_stage2b_report(raw: pd.DataFrame, config: Stage2bConfig, output_dir: Path) -> GateDecision:
    ...
    table = "\n".join(rows)
    (output_dir / "stage2b_report.md").write_text(
