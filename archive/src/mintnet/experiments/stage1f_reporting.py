"""Frozen gate evaluation and evidence rendering for the Stage 1f conditional-independence experiment."""
...
class GateDecision:
    """The immutable R2f decision derived from one run's raw evidence."""
...
def _target_sample_sizes(config: Stage1fConfig) -> tuple[int, ...]:
    """Use the frozen large-N gate cells that are present in this configuration.

    Per docs/stage1f_charter.md, the gate floor is N >= 750, raised from
...
def aggregate_stage1f(raw: pd.DataFrame) -> pd.DataFrame:
...
def _development_alpha_passes(rows: pd.DataFrame, alpha: float, config: Stage1fConfig) -> bool:
    """Require every individual (N, strength) cell to pass -- the R2f fix.
...
def select_alpha_pair(raw: pd.DataFrame, config: Stage1fConfig) -> tuple[float, float] | None:
...
def evaluate_stage1f_gate(raw: pd.DataFrame, config: Stage1fConfig) -> GateDecision:
...
def compute_calibration(raw: pd.DataFrame, config: Stage1fConfig) -> pd.DataFrame:
...
def write_stage1f_report(raw: pd.DataFrame, config: Stage1fConfig, output_dir: Path) -> GateDecision:
    """Write all aggregate evidence and the frozen R2f decision for one run."""
    output_dir.mkdir(parents=True, exist_ok=True)
    aggregate = aggregate_stage1f(raw)
    aggregate.to_csv(output_dir / "aggregate_metrics.csv", index=False)
    decision = evaluate_stage1f_gate(raw, config)
    ...
    (output_dir / "stage1f_report.md").write_text(
        "# Stage 1f Conditional-Independence Motif Validation Report\n\n"
