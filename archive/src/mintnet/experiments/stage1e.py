"""Deterministic raw-evidence runner for the frozen Stage 1e conditional-independence experiment."""
...
def load_stage1e_config(path: Path) -> Stage1eConfig:
    """Load a Stage 1e configuration, retaining the path used to resolve it."""
    with path.open(encoding="utf-8") as stream:
        values = yaml.safe_load(stream)
    if not isinstance(values, dict):
        raise ValueError("Stage 1e configuration must be a mapping")
...
def _repository_root(config: Stage1eConfig) -> Path:
...
def _resolved_config(config: Stage1eConfig) -> dict[str, object]:
    return {
...
def _write_evidence(
    config: Stage1eConfig, output_dir: Path, raw: pd.DataFrame, runtime_seconds: float
) -> None:
    ...
    charter = repository_root / "docs/stage1e_charter.md"
...
def run_stage1e(config: Stage1eConfig, output_dir: Path) -> pd.DataFrame:
    """Run configured Stage 1e conditions and persist only raw evidence."""
    ...
    # every CLI invocation leaves the aggregate R2e gate evidence alongside it.
    from mintnet.experiments.stage1e_reporting import write_stage1e_report

    write_stage1e_report(raw, config, output_dir)
    return raw
...
def main() -> None:
    ...
    run_stage1e(load_stage1e_config(arguments.config), arguments.output)
