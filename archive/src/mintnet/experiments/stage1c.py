"""Deterministic raw-evidence runner for the frozen Stage 1c conditional-independence experiment."""
...
def load_stage1c_config(path: Path) -> Stage1cConfig:
    """Load a Stage 1c configuration, retaining the path used to resolve it."""
    with path.open(encoding="utf-8") as stream:
        values = yaml.safe_load(stream)
    if not isinstance(values, dict):
        raise ValueError("Stage 1c configuration must be a mapping")
...
def _repository_root(config: Stage1cConfig) -> Path:
...
def _resolved_config(config: Stage1cConfig) -> dict[str, object]:
    return {
...
def _write_evidence(
    config: Stage1cConfig, output_dir: Path, raw: pd.DataFrame, runtime_seconds: float
) -> None:
    ...
    charter = repository_root / "docs/stage1c_charter.md"
...
def run_stage1c(config: Stage1cConfig, output_dir: Path) -> pd.DataFrame:
    """Run configured Stage 1c conditions and persist only raw evidence."""
    ...
    # every CLI invocation leaves the aggregate R2c gate evidence alongside it.
    from mintnet.experiments.stage1c_reporting import write_stage1c_report

    write_stage1c_report(raw, config, output_dir)
    return raw
...
def main() -> None:
    ...
    run_stage1c(load_stage1c_config(arguments.config), arguments.output)
