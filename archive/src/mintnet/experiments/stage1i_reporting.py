"""Per-N gate evaluation and evidence rendering for the Stage 1i experiment.

Unlike every prior Stage 1 charter, R2i selects and validates an alpha pair
independently for each sample size rather than requiring one pair to pass
across a pooled range of N (docs/stage1i_charter.md).
"""
...
class NDecision:
    """The immutable R2i decision for a single sample size."""
...
@dataclass(frozen=True)
class GateDecision:
    """The immutable R2i decision: one status per sample size, not a single global one."""
...
def aggregate_stage1i(raw: pd.DataFrame) -> pd.DataFrame:
...
def _alpha_margin_at_n(rows: pd.DataFrame, n: int, alpha: float, config: Stage1iConfig) -> float | None:
...
def select_alpha_pair_for_n(
    raw: pd.DataFrame, n: int, config: Stage1iConfig
...
def evaluate_n(raw: pd.DataFrame, n: int, config: Stage1iConfig) -> NDecision:
...
def evaluate_stage1i_gate(raw: pd.DataFrame, config: Stage1iConfig) -> GateDecision:
...
def compute_calibration(raw: pd.DataFrame, config: Stage1iConfig) -> pd.DataFrame:
...
def write_stage1i_report(raw: pd.DataFrame, config: Stage1iConfig, output_dir: Path) -> GateDecision:
    """Write all aggregate evidence and the frozen R2i per-N decision table."""
    output_dir.mkdir(parents=True, exist_ok=True)
    aggregate = aggregate_stage1i(raw)
    aggregate.to_csv(output_dir / "aggregate_metrics.csv", index=False)
    decision = evaluate_stage1i_gate(raw, config)
    ...
    table = "\n".join(rows)
    (output_dir / "stage1i_report.md").write_text(
        "# Stage 1i Per-N Alpha Selection Report\n\n"
