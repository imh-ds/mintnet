"""Frozen gate evaluation and evidence rendering for the Stage 1d conditional-independence experiment."""
...
from mintnet.experiments.stage1d import Stage1dConfig
...
class GateDecision:
    """The immutable R2d decision derived from one run's raw evidence."""
...
def _target_sample_sizes(config: Stage1dConfig) -> tuple[int, ...]:
    """Use the frozen large-N gate cells that are present in this configuration.

    Per docs/stage1d_charter.md, the gate floor is N >= 750, raised from
...
def aggregate_stage1d(raw: pd.DataFrame) -> pd.DataFrame:
...
def _development_alpha_passes(rows: pd.DataFrame, alpha: float, config: Stage1dConfig) -> bool:
    """Require every individual (N, strength) cell to pass -- the R2d fix.
...
def select_alpha_pair(raw: pd.DataFrame, config: Stage1dConfig) -> tuple[float, float] | None:
...
def evaluate_stage1d_gate(raw: pd.DataFrame, config: Stage1dConfig) -> GateDecision:
...
def compute_calibration(raw: pd.DataFrame, config: Stage1dConfig) -> pd.DataFrame:
...
def write_stage1d_report(raw: pd.DataFrame, config: Stage1dConfig, output_dir: Path) -> GateDecision:
    """Write all aggregate evidence and the frozen R2d decision for one run."""
    output_dir.mkdir(parents=True, exist_ok=True)
    aggregate = aggregate_stage1d(raw)
    aggregate.to_csv(output_dir / "aggregate_metrics.csv", index=False)
    decision = evaluate_stage1d_gate(raw, config)
    ...
    (output_dir / "stage1d_report.md").write_text(
        "# Stage 1d Conditional-Independence Motif Validation Report\n\n"
