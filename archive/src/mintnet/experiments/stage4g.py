"""Deterministic raw-evidence runner for the frozen Stage 4g held-out
alpha(N) validation (sequential engine, overlap shape). See
docs/stage4g_charter.md.

Tests exactly one alpha per held-out sample size -- the value predicted
by the formula fitted in `mintnet.experiments.stage4g_fit` from Stage
4e's own evidence -- rather than searching a grid, mirroring Stage 1j's
own held-out validation design. Reuses Stage 1L's overlap DGP/ground
truth, Stage 4b/4d/4e/4f's exact seed derivation, and Stage 4a's
sequential engine (`sequential_screen_and_prune_detailed`) unmodified.
"""
...
def load_stage4g_config(path: Path) -> Stage4gConfig:
    """Load a Stage 4g configuration, retaining the path used to resolve it."""
    with path.open(encoding="utf-8") as stream:
        values = yaml.safe_load(stream)
    if not isinstance(values, dict):
        raise ValueError("Stage 4g configuration must be a mapping")
...
def _condition_seed(master_seed: int, sample_index: int, replicate: int) -> int:
    # Matches Stage 4b/4d/4e/4f's exact seed derivation for overlap's shape
    # index; held-out N here were never simulated by any prior charter.
    sequence = np.random.SeedSequence([master_seed, _OVERLAP_SHAPE_INDEX, sample_index, replicate])
    return int(sequence.generate_state(1)[0])


def _repository_root(config: Stage4gConfig) -> Path:
...
def _resolved_config(
    config: Stage4gConfig, selected_form_name: str, predicted_alpha_by_n: dict[int, float]
) -> dict[str, object]:
    return {
...
def _write_evidence(
    ...
    (output_dir / "fitting_points.json").write_text(
        json.dumps({"points": [{"n": n, "alpha_star": a} for n, a in fitting_points]}, indent=2) + "\n",
        encoding="utf-8",
    )

    repository_root = _repository_root(config)
    charter = repository_root / "docs/stage4g_charter.md"
...
def run_stage4g(config: Stage4gConfig, stage4e_raw_path: Path, output_dir: Path) -> pd.DataFrame:
    """Fit alpha(N) from Stage 4e's evidence and validate it, fresh, at the
    configured held-out sample sizes."""
    ...
    _write_evidence(
        config, output_dir, raw, fitting_points, selected.name, predicted_alpha_by_n,
        stage4e_raw_path, time.perf_counter() - run_started,
    )
    from mintnet.experiments.stage4g_reporting import write_stage4g_report

    write_stage4g_report(raw, config, fitting_points, selected, output_dir)
    return raw
...
def main() -> None:
    ...
    run_stage4g(load_stage4g_config(arguments.config), arguments.stage4e_raw_evidence, arguments.output)
