"""Reporting-only runner for the frozen Stage 1d per-cell selection charter.

Reuses Stage 1c's raw evidence unmodified rather than re-simulating: the
DGP, mechanism, seeds, and alpha grid are byte-for-byte identical to R2c
(docs/stage1c_charter.md); only the development-selection rule changes
(docs/stage1d_charter.md).
"""
...
def load_stage1d_config(path: Path) -> Stage1dConfig:
    """Load a Stage 1d configuration, retaining the path used to resolve it."""
    with path.open(encoding="utf-8") as stream:
        values = yaml.safe_load(stream)
    if not isinstance(values, dict):
        raise ValueError("Stage 1d configuration must be a mapping")
...
def _repository_root(config: Stage1dConfig) -> Path:
...
def _resolved_config(config: Stage1dConfig) -> dict[str, object]:
    return {
...
def _write_evidence(
    ...
    charter = repository_root / "docs/stage1d_charter.md"
...
def run_stage1d(config: Stage1dConfig, raw_metrics_path: Path, output_dir: Path) -> pd.DataFrame:
    """Re-evaluate R2c's raw evidence under the frozen R2d per-cell selection rule."""
    ...
    # every CLI invocation leaves the aggregate R2d gate evidence alongside it.
    from mintnet.experiments.stage1d_reporting import write_stage1d_report

    write_stage1d_report(raw, config, output_dir)
    return raw
...
def main() -> None:
    ...
    run_stage1d(load_stage1d_config(arguments.config), arguments.raw_evidence, arguments.output)
