"""Deterministic raw-evidence runner for the frozen Stage 1h conditional-independence experiment."""
...
def load_stage1h_config(path: Path) -> Stage1hConfig:
    """Load a Stage 1h configuration, retaining the path used to resolve it."""
    with path.open(encoding="utf-8") as stream:
        values = yaml.safe_load(stream)
    if not isinstance(values, dict):
        raise ValueError("Stage 1h configuration must be a mapping")
...
def _repository_root(config: Stage1hConfig) -> Path:
...
def _resolved_config(config: Stage1hConfig) -> dict[str, object]:
    return {
...
def _write_evidence(
    config: Stage1hConfig, output_dir: Path, raw: pd.DataFrame, runtime_seconds: float
) -> None:
    ...
    charter = repository_root / "docs/stage1h_charter.md"
...
def run_stage1h(config: Stage1hConfig, output_dir: Path) -> pd.DataFrame:
    """Run configured Stage 1h conditions and persist only raw evidence."""
    ...
    # every CLI invocation leaves the aggregate R2h gate evidence alongside it.
    from mintnet.experiments.stage1h_reporting import write_stage1h_report

    write_stage1h_report(raw, config, output_dir)
    return raw
...
def main() -> None:
    ...
    run_stage1h(load_stage1h_config(arguments.config), arguments.output)
