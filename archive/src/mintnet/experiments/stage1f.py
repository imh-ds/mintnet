"""Deterministic raw-evidence runner for the frozen Stage 1f conditional-independence experiment."""
...
def load_stage1f_config(path: Path) -> Stage1fConfig:
    """Load a Stage 1f configuration, retaining the path used to resolve it."""
    with path.open(encoding="utf-8") as stream:
        values = yaml.safe_load(stream)
    if not isinstance(values, dict):
        raise ValueError("Stage 1f configuration must be a mapping")

    config = Stage1fConfig(
...
def _condition_seed(
    ...
    # Matches Stage 1/1b's seed derivation exactly, keyed on each sample
    # size's position in the frozen sample_sizes list. Appending new N
    # values after the existing six reproduces R2/R2b's exact data at every
    # previously tested N and adds genuinely new data only at the new N.
...
def _repository_root(config: Stage1fConfig) -> Path:
...
def _resolved_config(config: Stage1fConfig) -> dict[str, object]:
    return {
...
def _write_evidence(
    config: Stage1fConfig, output_dir: Path, raw: pd.DataFrame, runtime_seconds: float
) -> None:
    ...
    charter = repository_root / "docs/stage1f_charter.md"
...
def run_stage1f(config: Stage1fConfig, output_dir: Path) -> pd.DataFrame:
    """Run configured Stage 1f conditions and persist only raw evidence."""
    ...
    # every CLI invocation leaves the aggregate R2f gate evidence alongside it.
    from mintnet.experiments.stage1f_reporting import write_stage1f_report

    write_stage1f_report(raw, config, output_dir)
    return raw
...
def main() -> None:
    ...
    run_stage1f(load_stage1f_config(arguments.config), arguments.output)
