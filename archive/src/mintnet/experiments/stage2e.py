"""Deterministic raw-evidence runner for the frozen Stage 2e p=30
screening experiment. See docs/stage2e_charter.md.

Reuses Stage 2's config schema, seeding, and scoring unmodified
(`mintnet.experiments.stage2.Stage2Config`, `load_stage2_config`,
`_condition_seed`, `_score_flagged` -- all already generic over
`noise_count`/`p`, needing no DGP-specific change to run at `p=30`).
This module exists as its own file, rather than just a new config
passed to `mintnet.experiments.stage2`, because Stage 2's own
`_write_evidence` and report-writer hardcode `docs/stage2_charter.md`
and "Stage 2 Candidate-Edge Screening Report" -- reusing those
unmodified would hash and label this charter's evidence as Stage 2's,
...
from mintnet.experiments.stage2 import Stage2Config, _condition_seed, _score_flagged, load_stage2_config
from mintnet.screening import benjamini_hochberg_threshold, compute_pairwise_screening_evidence, screen_uncorrected
...
def _write_evidence(config: Stage2Config, output_dir: Path, raw: pd.DataFrame, runtime_seconds: float) -> None:
    ...
    charter = repository_root / "docs/stage2e_charter.md"
...
def run_stage2e(config: Stage2Config, output_dir: Path) -> pd.DataFrame:
    """Run configured Stage 2e conditions and persist only raw evidence."""
    ...
    from mintnet.experiments.stage2e_reporting import write_stage2e_report

    write_stage2e_report(raw, config, output_dir)
    return raw
...
def main() -> None:
    ...
    run_stage2e(load_stage2_config(arguments.config), arguments.output)
