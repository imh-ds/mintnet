"""Deterministic raw-evidence runner for the frozen Stage 1k hub-conditioning experiment."""
...
HUB = 0
CHILDREN = (1, 2, 3)
TRUE_EDGES: tuple[tuple[int, int], ...] = tuple((HUB, child) for child in CHILDREN)
INDIRECT_EDGES: tuple[tuple[int, int], ...] = tuple(combinations(CHILDREN, 2))
...
def load_stage1k_config(path: Path) -> Stage1kConfig:
    """Load a Stage 1k configuration, retaining the path used to resolve it."""
    with path.open(encoding="utf-8") as stream:
        values = yaml.safe_load(stream)
    if not isinstance(values, dict):
        raise ValueError("Stage 1k configuration must be a mapping")
...
def _condition_seed(config: Stage1kConfig, sample_index: int, replicate: int) -> int:
    sequence = np.random.SeedSequence([config.master_seed, sample_index, replicate])
    return int(sequence.generate_state(1)[0])


def _repository_root(config: Stage1kConfig) -> Path:
...
def _resolved_config(config: Stage1kConfig, alpha_by_n: dict[int, float]) -> dict[str, object]:
    return {
...
def _write_evidence(
    config: Stage1kConfig, output_dir: Path, raw: pd.DataFrame, alpha_by_n: dict[int, float], runtime_seconds: float
) -> None:
    ...
    charter = repository_root / "docs/stage1k_charter.md"
...
def run_stage1k(config: Stage1kConfig, output_dir: Path) -> pd.DataFrame:
    """Run configured Stage 1k conditions and persist only raw evidence."""
    ...
    for sample_index, n in enumerate(config.sample_sizes):
        ...
        for replicate in range(config.replicates):
            ...
            try:
                data = sample_hub(n, config.strength, children=3, rng=np.random.default_rng(seed))
                indirect_pruned = 0
                for a, b in INDIRECT_EDGES:
                    other_child = next(c for c in CHILDREN if c not in (a, b))
                    retained = prune_pair(data, a, b, [HUB, other_child], alpha)
                    if not retained:
                        indirect_pruned += 1
                true_retained = 0
                for hub, child in TRUE_EDGES:
                    others = [c for c in CHILDREN if c != child]
                    retained = prune_pair(data, hub, child, others, alpha)
                    if retained:
                        true_retained += 1
    ...
    from mintnet.experiments.stage1k_reporting import write_stage1k_report

    write_stage1k_report(raw, config, output_dir)
    return raw
...
def main() -> None:
    ...
    run_stage1k(load_stage1k_config(arguments.config), arguments.output)
